...
data class VerifyApproveMessage(val cipher: String, val publicKey: String?, val initializationVector: String?) {

    fun toSocketData() = JsonObject()
        .also {
            it.addProperty("cipher", cipher)
            it.addProperty("publicKey", publicKey)
            it.addProperty("initializationVector", initializationVector)
